...
parser.add_argument('--ig_window', type=int, default=20, help='wide timestep window length')
...
# ---------------- IG Decode (full sequence, dynamically scaled, window-aware) ----------------
if __name__ == "__main__":
    # cuDNN safety
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True

    print(f"[INFO] Running Integrated Gradients (IG Decode) for {args.dataset}")
    print(f"[INFO] IG window length = {args.ig_window} / Total sequence = {args.len_sw}")
    ...
    # ---------------- Load pretrained backbone + classifier ----------------
    ...
    # ---------------- Parameters ----------------
    T, C = args.ig_window, args.n_feature   # <-- use ig_window instead of len_sw
    ...
    for xb, yb, _ in test_loader:
        for i in range(0, xb.size(0), MICRO):
            ...
            # 🔹 Apply window cropping if shorter than sequence
            if args.ig_window < batch.shape[1]:
                batch = batch[:, :args.ig_window, :]

            # normalize per-channel (stability)
            mean = batch.mean(dim=(0, 1), keepdim=True)
            stds = batch.std(dim=(0, 1), keepdim=True) + 1e-8
            batch = ((batch - mean) / stds).requires_grad_(True)
    ...
    # ---------------- Aggregate & Normalize (Dynamic Scaling 4–10%) ----------------
    all_attr = np.concatenate(all_attr, axis=0)     # (N, T, C)
    attr_abs = np.abs(all_attr).mean(axis=0)        # (T, C)
    
    # Normalize within channel, then scale by channel weight
    ...
    out_csv = f'./results/ig_outputs/ig_decode_dynamic_{args.dataset}_{args.framework}_{args.backbone}_win{args.ig_window}.csv'

    rows = []
    for i, v in enumerate(flat_importance):
        t, ch, axis_name = decode_feature_index(i, (T, C))
    ...
    print(f"[INFO] Top-5 most influential features (IG Decode, dynamically scaled):")
    for idx in top_idx:
        t, ch, axis_name = decode_feature_index(idx, (T, C))
