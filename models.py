...
class Embeddings(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        in_dim = getattr(cfg, "feature_num", None)
        out_dim = (
            getattr(cfg, "hidden_size", None)
            or getattr(cfg, "hidden_dim", None)
            or getattr(cfg, "d_model", None)
            or getattr(cfg, "hidden", None)
        )
        if in_dim is None:
            raise AttributeError("[Embeddings] missing feature_num")
        if out_dim is None:
            raise AttributeError("[Embeddings] missing hidden size (hidden_size/hidden_dim/d_model/hidden)")
    ...
    def forward(self, x):
        ...
        x = x.to(dev, dtype=torch.float32, non_blocking=True).contiguous()
        if torch.isnan(x).any() or torch.isinf(x).any():
            x = torch.nan_to_num(x)

        if x.size(-1) != self.lin.in_features:
            raise RuntimeError(
                f"[Embeddings] Expected last dim {self.lin.in_features}, got {x.size(-1)}; shape={tuple(x.shape)}"
            )

        # --- GPU first; if old cuBLAS fails, fall back to CPU matmul ---
        try:
            e = self.lin(x)
        except RuntimeError as err:
            if "CUBLAS_STATUS_EXECUTION_FAILED" in str(err):
                # fallback: run the linear on CPU, then move result back
                w_dev = self.lin.weight.device
                b = self.lin.bias
                self.lin = self.lin.to("cpu")
                e = self.lin(x.to("cpu"))
                e = e.to(w_dev if isinstance(w_dev, torch.device) else dev)
                # move layer back where it was so subsequent calls still work
                self.lin = self.lin.to(dev)
            else:
                raise
...
class Transformer(nn.Module):
    """
    Simplified Transformer encoder wrapper.
    Automatically adjusts hidden size to be divisible by number of heads.
    """

    def __init__(self, cfg):
        super(Transformer, self).__init__()

        # Get hidden and head configuration
        hidden_dim = (
            getattr(cfg, "hidden_size", None)
            or getattr(cfg, "hidden_dim", None)
            or getattr(cfg, "d_model", None)
        ...
        n_heads = getattr(cfg, "n_heads", getattr(cfg, "num_heads", 1))
        n_layers = getattr(cfg, "n_layers", getattr(cfg, "num_layers", 1))
        dropout = getattr(cfg, "dropout", 0.1)

        if hidden_dim is None:
            raise AttributeError("[Transformer] Missing hidden dimension in config")

        # Auto-fix for non-divisible hidden sizes
        if hidden_dim % n_heads != 0:
            new_hidden = math.ceil(hidden_dim / n_heads) * n_heads
            print(
                f"[Transformer] Warning: hidden_dim={hidden_dim} not divisible by n_heads={n_heads}. "
                f"Adjusted hidden_dim to {new_hidden}."
            )
    ...
    def forward(self, x):
        # Input: [batch, seq_len, feature_num]
        x = self.embed(x)  # -> [batch, seq_len, hidden_dim]

        # Sanity check before Transformer
        if torch.isnan(x).any() or torch.isinf(x).any():
            print("[Transformer] Warning: NaN/Inf detected pre-encoder, cleaning.")
            x = torch.nan_to_num(x)
