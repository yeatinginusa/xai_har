...
class PretrainModelConfig(NamedTuple):
    hidden_size: int = 0  # alias for hidden, used by models ✅
    hidden: int = 0        # Dimension of Hidden Layer in Transformer Encoder
    hidden_ff: int = 0     # Dimension of Intermediate Layers in Feedforward Net
    feature_num: int = 0   # Factorized embedding parameterization
    ...
    @classmethod
    def from_json(cls, js):
        # ensure backward compatibility if JSON has "hidden" but not "hidden_size"
        if "hidden_size" not in js and "hidden" in js:
            js["hidden_size"] = js["hidden"]
        return cls(**js)
