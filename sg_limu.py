...
parser = argparse.ArgumentParser(description="SmoothGrad (Saliency) explanation for LIMU-BERT")
...
# SmoothGrad knobs
parser.add_argument("--sg_nsamples", type=int, default=25,
                    help="Number of noise samples for SmoothGrad")
parser.add_argument("--sg_sigma", type=float, default=0.1,
                    help="Noise std as fraction of input std (per-channel)")
...
if __name__ == "__main__":
    ...
    def forward_fn(x):
        feats = model(x)              # (B, D) or (B, T, D)
        if feats.ndim == 3:
            feats = feats.mean(dim=1) # pool over time if sequence output
        logits = classifier(feats)    # (B, n_class)
        return logits

    # ---------------- Saliency + NoiseTunnel (SmoothGrad) ----------------
    ...
    for xb, yb, _ in test_loader:
        for i in range(0, xb.size(0), MICRO):
            ...
            # Normalize per-channel (stabilizes gradients)
            stds = batch.std(dim=(0, 1), keepdim=True) + 1e-8
            batch = (batch / stds).requires_grad_(True)

            # Per-channel noise std = sg_sigma * channel std (≈1 after normalization → sg_sigma absolute)
            # stype='gaussian' gives classic SmoothGrad (average of |grad| over noisy samples).
            attributions = sg.attribute(
                batch,
                nt_type='smoothgrad',
                nt_samples=max(1, args.sg_nsamples),
                stdevs=args.sg_sigma,
                target=target,
                abs=True
            )  # shape: (B, T, C)
    ...
    # per-channel normalization to % (matches your IG output style)
    ...
    out_csv = f'./results/sg_outputs/sg_global_{args.dataset}_limubert.csv'
    ...
    print(f"\n[INFO] Top 5 most influential features (SG, LIMU-BERT):")
    for idx in top_idx:
