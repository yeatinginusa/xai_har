...
if __name__ == "__main__":
    ...
    # Safer cuDNN settings (helps with fragmentation on some GPUs)
    ...
    # ================================================================
    # Load pretrained backbone & linear classifier
    # ================================================================
    ...
    # ================================================================
    # Forward function (returns probabilities)
    # ================================================================
    def forward_fn(x):
        ...
        if isinstance(feats, (tuple, list)):
            feats = feats[-1]
        if feats.ndim == 3:
            if feats.shape[1] == args.len_sw:
                feats = feats.mean(dim=1)
            elif feats.shape[2] == args.len_sw:
                feats = feats.mean(dim=2)
        logits = classifier(feats)
        return torch.softmax(logits, dim=1)

    # Base method + SmoothGrad wrapper
    from captum.attr import IntegratedGradients, NoiseTunnel
    ig = IntegratedGradients(forward_fn)
    sg = NoiseTunnel(ig)

    # ================================================================
    # SmoothGrad params tuned for low memory
    # ================================================================
    SG_MICROBATCH = max(1, args.batch_size // 8)   # split batch further
    NT_SAMPLES = 10                                 # was 25
    N_STEPS = 20                                    # was 50
    INT_BATCH = 8                                   # split IG steps on device
    NOISE_STD = 0.05                                # gentle noise
    print(f"[INFO] SG micro-batch={SG_MICROBATCH}, nt_samples={NT_SAMPLES}, n_steps={N_STEPS}, internal_batch_size={INT_BATCH}")

    # ================================================================
    # Compute SmoothGrad attributions (memory-optimized)
    # ================================================================
    all_attr = []

    for inputs, labels, _ in test_loader:
        # process in micro-batches to reduce peak memory
        for i in range(0, inputs.size(0), SG_MICROBATCH):
            batch = inputs[i:i+SG_MICROBATCH].to(DEVICE)
            labels_b = labels[i:i+SG_MICROBATCH].to(DEVICE)
            target = labels_b.argmax(dim=1) if labels_b.ndim > 1 else labels_b

            # normalize per-channel (helps stability)
            stds = batch.std(dim=(0, 1), keepdim=True) + 1e-8
            batch = (batch / stds).requires_grad_(True)
            baseline = torch.zeros_like(batch)

            # Autocast to cut activation memory; Captum handles grads fine
            with torch.cuda.amp.autocast(enabled=True):
                attr = sg.attribute(
                    ...
                    stdevs=NOISE_STD,
                    n_steps=N_STEPS,
                    internal_batch_size=INT_BATCH,  # <— splits IG path
                )

            all_attr.append(attr.detach().cpu().numpy())

            # explicit cleanup between micro-batches
            del batch, labels_b, baseline, attr
            torch.cuda.empty_cache()

    # ================================================================
    # Aggregate, normalize, save, and print Top-5
    # ================================================================
    all_attr = np.concatenate(all_attr, axis=0)         # (N, T, C)
    attr_abs = np.abs(all_attr).mean(axis=0)            # (T, C)
    channel_sum = attr_abs.sum(axis=0, keepdims=True) + 1e-8
    attr_norm = 100.0 * (attr_abs / channel_sum)        # per-channel %
    flat_importance = attr_norm.flatten()

    T, C_eff = args.len_sw, args.n_feature
    decoded_features, decoded_weights = [], []
    for idx, imp in enumerate(flat_importance):
        t = idx // C_eff
        ch = idx % C_eff
        decoded_features.append(f"f{idx} (t={t}, ch={ch})")
        decoded_weights.append(round(float(imp), 4))

    sorted_global = sorted(zip(decoded_features, decoded_weights), key=lambda x: x[1], reverse=True)

    os.makedirs('./results/sg_outputs', exist_ok=True)
    out_csv = f'./results/sg_outputs/sg_global_{args.dataset}_{args.framework}_{args.backbone}.csv'
    pd.DataFrame(sorted_global, columns=["Feature", "Importance (%)"]).to_csv(out_csv, index=False)
    print(f"[INFO] Saved SG results → {out_csv}")
    
    print(f"\n[INFO] Running SmoothGrad on {args.dataset} | {args.framework} | {args.backbone}")
    print(f"[INFO] Top 5 most influential features (SmoothGrad) for {args.dataset}:")
